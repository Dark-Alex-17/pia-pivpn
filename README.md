...
In order to have both VPNs running at once without them interfering with one another, they each run using a separate protocol:
...
Ensure you have your username and password stored in a text file in `/etc/openvpn/auth.txt` and format it like below:
```text
p01234567
password123!
```
...
To start the PIA VPN (OpenVPN), create the port forwarding for PiVPN, and to start PiVPN (WireGuard), run `./startup_vpn.sh`

### Note: that you will have to restart the VPN every 3 months as that's how long the port forwarding lasts!! You'll have to re-attach any devices with new creds as well!
